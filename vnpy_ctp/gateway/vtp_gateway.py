...
class VtpMdApi():
    ...
    def parse_tick(self, msg_body: bytes) -> None:
        ...
        bid_price_2 = struct.unpack('<d', msg_body[248:256])[0]
        bid_volume_2 = struct.unpack('<l', msg_body[256:260])[0]
        bid_price_3 = struct.unpack('<d', msg_body[264:272])[0]
        bid_volume_3 = struct.unpack('<l', msg_body[272:276])[0]
        bid_price_4 = struct.unpack('<d', msg_body[280:288])[0]
        bid_volume_4 = struct.unpack('<l', msg_body[288:292])[0]
        bid_price_5 = struct.unpack('<d', msg_body[296:304])[0]
        bid_volume_5 = struct.unpack('<l', msg_body[304:308])[0]
        ask_price_1 = struct.unpack('<d', msg_body[312:320])[0]
        ask_volume_1 = struct.unpack('<l', msg_body[320:324])[0]
        ask_price_2 = struct.unpack('<d', msg_body[328:336])[0]
        ask_volume_2 = struct.unpack('<l', msg_body[336:340])[0]
        ask_price_3 = struct.unpack('<d', msg_body[344:352])[0]
        ask_volume_3 = struct.unpack('<l', msg_body[352:356])[0]
        ask_price_4 = struct.unpack('<d', msg_body[360:368])[0]
        ask_volume_4 = struct.unpack('<l', msg_body[368:372])[0]
